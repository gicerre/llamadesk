...
pub fn children(conn: &Connection, profile_id: &str, parent_id: Option<&str>) -> Result<Vec<Container>> {
    let mut statement = conn.prepare(
...
pub fn list_by_profile(conn: &Connection, profile_id: &str) -> Result<Vec<Container>> {
    let mut statement = conn.prepare(
        "SELECT * FROM containers WHERE profile_id = ?1 ORDER BY sort_order, name",
    )?;
...
pub fn assert_nesting_allowed(conn: &Connection, parent_kind: &str, child_kind: &str) -> Result<()> {
...
pub fn create(
    ...
    conn.execute(
        ...
        params![id, profile_id, parent_id, kind, name, slugify(name), sort_order],
    )?;
...
pub fn move_node(
    ...
    if let Some(target) = new_parent_id {
        ...
        assert_nesting_allowed(conn, &get(conn, target)?.kind, &current.kind)?;
    } else {
        assert_nesting_allowed(conn, "root", &current.kind)?;
    }

    let sort_order = ordering::place(conn, TABLE, "parent_id", new_parent_id, previous_id, next_id)?;
...
mod tests {
    ...
    fn level_update_distinguishes_untouched_from_inherit() {
        ...
        assert_eq!(resolve_level_update(Some("inherit"), Some("critical".into())), None);
        assert_eq!(
